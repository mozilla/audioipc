...
//               This value must be sent to the remote, so is updated to a `SerializableValue` via
//               `AssociateHandleForMessage::set_remote_handle`
//    - Unix: sendmsg transfers the handle to the remote process.
// 4. Message received and deserialized in target process.
//    - Windows: `AssociateHandleForMessage::Deserialization converts the `SerializableValue` into `Owned`, ready for use.
//    - Unix: Handle (with a new value in the target process) is received out-of-band via `recvmsg`
//            and converted to `Owned` via `AssociateHandleForMessage::set_owned_handle`.
#[derive(Debug)]
    ...
    fn take_handle_for_send(&mut self) -> RemoteHandle {
        match std::mem::replace(self, SerializableHandle::Empty) {
            SerializableHandle::Owned(handle, target_pid) => unsafe {
                RemoteHandle::new(handle.into_raw(), target_pid)
            },
...
impl RemoteHandle {
    ...
    pub unsafe fn new(handle: PlatformHandleType, _target: Option<u32>) -> Self {
        RemoteHandle {
            handle,
            #[cfg(windows)]
            target: _target.expect("target process required"),
            #[cfg(windows)]
            target_handle: None,
        }
    }
